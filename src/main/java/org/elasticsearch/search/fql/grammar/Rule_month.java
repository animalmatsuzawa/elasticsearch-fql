...
final public class Rule_month extends Rule
{
  public Rule_month(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_month parse(ParserContext context)
  {
    context.push("month");
        ...
        for (int i1 = 0; i1 < 1 && f1; i1++)
        {
          ...
          parsed = false;
          {
            ...
            if (parsed)
            {
              ...
              for (int i2 = 0; i2 < 1 && f2; i2++)
              {
                Rule rule = Terminal_NumericValue.parse(context, "%x30-32", "[\\x30-\\x32]", 1);
                if ((f2 = rule != null))
                {
                  a2.add(rule, context.index);
                  c2++;
    ...
    if (parsed)
    {
        rule = new Rule_month(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
