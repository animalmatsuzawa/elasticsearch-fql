...
public class XmlXContentFactory {
    ...
    /**
     * Returns a content builder using XML format ({@link org.elasticsearch.common.xcontent.XContentType#YAML}.
     */
    public static XContentBuilder xmlBuilder() throws IOException {
        return contentBuilder(XmlXContentType.XML);
    }
     ...
     */
    public static XContentBuilder contentBuilder(XmlXContentType type, OutputStream outputStream) throws IOException {
     ...
     */
    public static XContentBuilder contentBuilder(XmlXContentType type) throws IOException {
        if (type == XmlXContentType.JSON) {
...
//        if (length > 2 && first == '<' && content.charAt(1) == '?' && content.charAt(2) == 'x') {
...
//        }
    ...
    public static XmlXContentType xContentType(BytesReference bytes) {
        ...
        if (length > 2 && 
                first == SmileConstants.HEADER_BYTE_1 && 
                bytes.get(1) == SmileConstants.HEADER_BYTE_2 && 
