...
public class XmlXContentParser extends AbstractXContentParser {
    ...
    public XmlXContentParser(NamedXContentRegistry xContentRegistry, JsonParser parser) {
...
//        parser.isExpectedStartArrayToken();
        Token t = convertToken(parser.nextToken());
        return t;
    }
...
//        return readMap(this);
        ...
        for (; token == XContentParser.Token.FIELD_NAME; token = this.nextToken()) {
            ...
            Object value = this.readValue( token);
    ...
    public Object _list() throws IOException {
        XContentParser.Token token = this.currentToken();
        if (token == null) {
            token = this.nextToken();
        }
        ...
        if( token != XContentParser.Token.END_ARRAY )
        {
            ...
            for (; token != null && token != XContentParser.Token.END_ARRAY; token = this.nextToken()) {
    
                list.add(this.readValue(token));
            }
            return list;
        }
        else
    ...
    Object readValue( XContentParser.Token token) throws IOException {
